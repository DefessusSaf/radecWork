...
# Константа фокусного расстояния
FOCALLEN = 551
...
def extract_header(fits_file):
    ...
    # Вычисление масштаба в угловых секундах на пиксель
    scale = xpixsz / FOCALLEN*_u.rad.to('arcsec')*_u.um.to('mm')
    print(f"Calculated Scale: {scale} arcsec/pixel")
    
    return xpixsz, gain, exptime, scale
...
def main():
    ...
    # Извлекаем данные заголовка FITS и вычисляем масштаб
    xpixsz, gain, exptime, scale = extract_header(fits_file)
